# Generated by Django 3.1.2 on 2021-03-17 06:16
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('child_management', '0018_auto_20210312_0522'),
    ]
    ...
    migrations.RunSQL("""create or replace view dash_child_days_lastvisit_view as 
                            ...
                            extract( days from (age(now()::TIMESTAMP, cft.last_visit_date::TIMESTAMP))) as additional_days_last_visit
                            from child_management_child cc 
                            inner join (
                                select row_number() over (partition by child_id order by date_of_admission desc, id desc) as shelter_num, shelter_home_id, child_id, admission_number, date_of_admission
    ...
    migrations.RunSQL("""create or replace view dash_recommended_adoption_view as 
                            ...
                            inner join master_data_state mds on mdd.state_id = mds.id
                            left outer join (
                                select child_id, row_number() over (partition by child_id order by flagged_date desc, id desc) as flagging_num, flagged_status, flagged_date
    ...
    migrations.RunSQL("""create or replace view dash_child_cci_category as 
                            ...
                            mdd.id as district_id, mdd.name as district_name, 
                            sh.id as shelter_home_id, sh.name as shelter_homename, 
                            coalesce(sum(shch.child_count),0) as child_count,
                            coalesce(sum(cc.abandoned_count),0) as abandoned_count,
                            coalesce(sum(cc.fam_not_trace_count),0) as fam_not_trace_count,
                            coalesce(sum(cc.surrendered_count),0) as surrendered_count,
                            coalesce(sum(cc.was_trafficked_count),0) as was_trafficked_count,
                            coalesce(sum(cc.orp_no_guardian_count),0) as orp_no_guardian_count,
                            coalesce(sum(cc.orp_fit_guardian_count),0) as orp_fit_guardian_count,
                            coalesce(sum(cc.unfit_guardian_poverty_count),0) as unfit_guardian_poverty_count,
                            coalesce(sum(cc.unfit_guardian_addict_count),0) as unfit_guardian_addict_count,
                            coalesce(sum(cc.unfit_guardian_abuse_count),0) as unfit_guardian_abuse_count,
                            coalesce(sum(cc.unfit_guardian_criminal_count),0) as unfit_guardian_criminal_count,
                            coalesce(sum(cc.unfit_guardian_unsound_count),0) as unfit_guardian_unsound_count,
                            coalesce(sum(cc.unfit_guardian_needcare_count),0) as unfit_guardian_needcare_count,
                            coalesce(sum(cc.unfit_guardian_unwilling_count),0) as unfit_guardian_unwilling_count,
                            coalesce(sum(cc.fit_parent_count),0) as fit_parent_count,
                            coalesce(sum(cc.unfit_parent_poverty_count),0) as unfit_parent_poverty_count,
                            coalesce(sum(cc.unfit_parent_single_count),0) as unfit_parent_single_count,
                            coalesce(sum(cc.unfit_parent_addict_count),0) as unfit_parent_addict_count,
                            coalesce(sum(cc.unfit_parent_abuse_count),0) as unfit_parent_abuse_count,
                            coalesce(sum(cc.unfit_parent_criminal_count),0) as unfit_parent_criminal_count,
                            coalesce(sum(cc.unfit_parent_unsound_count),0) as unfit_parent_unsound_count,
                            coalesce(sum(cc.unfit_parent_needcare_count),0) as unfit_parent_needcare_count,
                            coalesce(sum(cc.unfit_parent_unwilling_count),0) as unfit_parent_unwilling_count
                            from master_data_shelterhome sh
                            inner join master_data_district mdd on sh.district_id = mdd.id and sh.active = 2 
                            inner join master_data_state mds on mdd.state_id = mds.id
                            inner join (
                                select shelter_home_id, count(distinct child_id) child_count
                                from (select row_number() over (partition by child_id order by date_of_admission desc, id desc) as shelter_num, shelter_home_id, child_id, admission_number, date_of_admission
                                    from  child_management_childshelterhomerelation 
                                    where active = 2) as x
                                where shelter_num  = 1
    ...
    migrations.RunSQL("""create or replace view dash_child_cci_stay_view as 
                            ...
                            extract( days  from (age(date_of_exit::TIMESTAMP, date_of_admission::TIMESTAMP))) as additional_days    
                            from child_management_child cc 
                            inner join (
                                select child_id, shelter_home_id, date_of_admission, coalesce(date_of_exit, coalesce(lag(date_of_admission,-1) over(partition by child_id order by date_of_admission asc, id asc)),now()) as date_of_exit
    ...
    migrations.RunSQL("""create or replace view dash_child_days_lastreview_view as 
                            ...
                            extract( days from (age(now()::TIMESTAMP, cch.last_review_date::TIMESTAMP))) as additional_days_last_review
                            from child_management_child cc 
                            inner join (
                                select row_number() over (partition by child_id order by date_of_admission desc, id desc) as shelter_num, shelter_home_id, child_id, admission_number, date_of_admission
                                from  child_management_childshelterhomerelation 
    ...
    migrations.RunSQL("""create or replace view rep_child_details_view as 
                            ...
                            cc.classification, 
                            (case when cs.stay_in_months is null then 'NA' 
                                when cs.stay_in_months = 0 and cs.additional_days < 30 then '< 1 month' 
                                else ((case when (cs.stay_in_months+(cs.additional_days/30)::int) >= 12 and (cs.stay_in_months+(cs.additional_days/30)::int) <24 then floor(((cs.stay_in_months+(floor(cs.additional_days/30)::numeric)::int)/12)::numeric) || ' year and '
                                        when (cs.stay_in_months+(cs.additional_days/30)::int) >= 24 then floor(((cs.stay_in_months+floor((cs.additional_days/30)::numeric)::int)/12)::numeric) || ' years and ' 
                                        else ''
                                        end) || ((cs.stay_in_months+floor((cs.additional_days/30)::numeric)::int)::int % 12) || ' month' || 
                                        (case when ((cs.stay_in_months+floor((cs.additional_days/30)::numeric)::int)::int % 12) > 1 then 's' else '' end)) end) as total_shelter_home_stay,
                            ...
                            left outer join (select child_id, coalesce(sum(stay_in_months),0) as stay_in_months, coalesce(sum(additional_days),0) as additional_days
                            ...
                            left outer join (
                                select child_id, row_number() over(partition by child_id order by flagged_date desc, id desc) as flag_num, reason_for_flagging as flagging_reason, flagged_date, flagged_status
                                from child_management_childflaggedhistory 
    ...
    migrations.RunSQL("""create or replace view rep_child_baseline_report as 
                            ...
                            left outer join (
                                select child_id, row_number() over (partition by child_id order by flagged_date desc, id desc) as flagging_num, flagged_status
                                from  child_management_childflaggedhistory 
                                where active = 2
